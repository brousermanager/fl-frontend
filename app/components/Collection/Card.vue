<template>
  <v-card elevation="8" class="ma-5" hover>
    <v-row @click="redirectToCollectionPage(podcastCollection.id)">
      <v-col cols="12">
        <v-img aspect-ratio="1/1" height="250" :src="podcastCollection.cover"/>
      </v-col>
      <v-col cols="12">
        <v-card-item>
    ...
    <v-card-actions>
      <v-btn color="orange-lighten-2" text="Descrizione" @click="show = !show"/>
...
const redirectToCollectionPage = (collectionId: string) => {
  return navigateTo(`/rubriche/${collectionId}`);
};
</script>
